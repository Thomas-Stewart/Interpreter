...
{
	if (isalpha(token[0])) //if first character is a letter
	{
		if (isKEYWORD(token)) //if already a keyword
			return false;
		else
			return true;
	}
...
{
	if (token[0] == 34 && token[token.size()-1] == 34)
		return true;
	else
		return false;
}
		...
		vectOfCategoryVects & categoryVectors)
{
	...
	{
		tokenVectors.resize(tokenVectors.size() + 1); //open up space to be filled in vector
		getPerLineTokenVectFromOneStringObject(linesOfStatements[i], tokenVectors[i]);
	}
	for (int i = 0; i < tokenVectors.size(); i++)
	{
		categoryVectors.resize(categoryVectors.size() + 1); //open up space to be filled in vector
